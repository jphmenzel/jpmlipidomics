...
%SKYLINE_RUNNER% --timestamp --dir="%ROOT_DIR%" --in="..\%SKYLINE_FILE%" --out="%MODEL_NAME%\OzFAD1_3_DDA_confirmed_DIA_RT_adjusted.sky" >> %LOG%

rem Do the analysis in the new location
%SKYLINE_RUNNER% --timestamp --dir="%ROOT_DIR%" --in="%MODEL_NAME%\OzFAD1_3_DDA_confirmed_DIA_RT_adjusted.sky" --import-transition-list=%ROOT_ANALYSIS_DIR%\jpmlipidomics_dda_vpw20_4_rt_shifted.csv --save --import-lockmass-positive=556.2771 --import-lockmass-tolerance=0.5 --import-all="%ROOT_ANALYSIS_DIR%\DIA_current_LCMS_dataset" --import-naming-pattern="_([^_]*)$" --save --report-add="%ROOT_ANALYSIS_DIR%\OzFAD1_black_box\skyline_report_vpw15.skyr" --report-conflict-resolution=overwrite --report-name=skyl_report_template_vpw15 --report-file="%ROOT_ANALYSIS_DIR%\OzFAD1_3_DDA_confirmed_DIA_RT_adjusted.csv" --report-invariant >> %LOG%
