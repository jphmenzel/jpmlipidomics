...
# begin sort transitions in writelist
sortall=0
if sortall==1:
	print('Established filter step is complete.')
	backupwritelist=writelist

	r=0
	while r<len(writelist[1]):
		e=writelist[1][r] # Precursorname		# begin determine which row to start (r) and to end (s)
		s=r+1
		st=0
		while st<1:
		...
		if s<(len(writelist[1])-1):
			k=s+1
			while writelist[4][r]==writelist[4][k]:	# same FA group
				#development paused			##########################################################################################################
				# go through other blocks, if same dbs, and if close together, merge into one instance, if same dbs, far enough apart cut and past k species after r species
		...
		k=k+1
# end sort transitions in writelist
