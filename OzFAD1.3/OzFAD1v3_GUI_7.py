...
#  TO TURN THIS FILE INTO A .EXE FILE RUN IN TERMINAL: pyinstaller --onefile -w OzFAD1v3_GUI_6.py	# all bundled in one file
#  TO TURN THIS FILE INTO A .EXE FILE RUN IN TERMINAL: pyinstaller --onedir -w OzFAD1v3_GUI_6.py	# all bundled in one folder
# EXE file will execute correctly, only when OzFAD_logo_HR.png etc is in the same folder as the exe file
...
if testrun==1:
	...
	resized_orangearrow=orangearrow.resize((owd, oht), Image.ANTIALIAS)
...
def frame2():
	...
	def run_discovery():
		...
		# label to display that calculation step is complete
		canvas3=tk.Canvas(zframe, width=850, height=30, bg='white', highlightthickness=0)  # bg='white'
		canvas3.place(x=xoffset2+0, y=yposid+100, width=850, height=5)
		lblyt='The calculation is complete.'
		lbly=tk.Label(text=lblyt, fg='black', bg='white', width=len(lblyt), height=1, font=('Helvetica 10 bold'), anchor='w')
		lbly.place(x=xoffset2+20, y=yposid+100, height=20, width=800)
