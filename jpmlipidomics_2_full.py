...
#  NOTES: filters precurso results and builds large list with pandas. export to csv 
import math
import os
import openpyxl
import pandas as pd
				...
				if (abs(targetrtlist[i]-targetrtlist[j]))<(targetstepwidth-0.000001):
					if targetmzlist[i]==targetmzlist[j]:
						...
						if thirdfound==0:
							...
							# three or more entries at overlapping RT with different m/z # identify range, modify species within range
							tmzi=targetmzlist[i]
							tmzj=targetmzlist[j]
							kf=0
							fourthfound=0
													...
													while kh<(len(targetrtlist)):
														if abs(targetrtlist[kh]-targetrtlist[i])<(targetstepwidth-0.000001):
															if targetmzlist[kh]==tmzi:
																...
																print('############ WTF #################### EDIT CODE FOR OVERLAP OF 10 #######')
														else:
